...
export default function Reportes() {
  ...
  async function cargarDatos() {
    ...
    try {
      const [ventasRes, productosRes, cajaRes] = await Promise.all([
        fetch("/ventas/"),
        fetch("/productos/"),
        fetch("/caja/"),
      ])
      ...
      setMovimientosCaja(Array.isArray(cajaData) ? cajaData : [])
    } catch (error) {
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <h3>💰 Total vendido</h3>
          <h2>{dinero(totalVendido)}</h2>
          <p>{cantidadVentas} ventas</p>
        </div>
